...
public class Node:Grid{
  ...
  public Node(ParsedJsonObj parsed_json_obj,CapricornAccount cpr_account,Config config,SignalPipe signal_pipe){
    ...
    text_drawing_box=new TextDrawingBox(this.parsed_json_obj,this.cpr_account,this.config,this.signal_pipe);
  ...
  protected void set_bg_color(){
    if(parsed_json_obj.is_mine){
      this.override_background_color(StateFlags.NORMAL,config.color_profile.mine_bg_rgba);
    }else{
      switch(parsed_json_obj.type){
        case ParsedJsonObjType.DELETE:this.override_background_color(StateFlags.NORMAL,config.color_profile.delete_bg_rgba);
        break;
        case ParsedJsonObjType.RETWEET:this.override_background_color(StateFlags.NORMAL,config.color_profile.retweet_bg_rgba);
        break;
        default:
        switch(parsed_json_obj.tweet_type){
          case TweetType.REPLY:this.override_background_color(StateFlags.NORMAL,config.color_profile.reply_bg_rgba);
          break;
