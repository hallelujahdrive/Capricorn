...
public class MediaPage:Frame{
  ...
  private void open_url_button_clicked_cb(Button open_url_button){
    for(int i=0;i<media_array.length;i++){
      open_url(media_array[i].expanded_url);
    }
  }
  ...
  public MediaPage(Node tweet_node,url[] media_array){
    ...
    for(int i=0;i<this.media_array.length;i++){
      photo_box_array.append_val(new PhotoBox(i,this.media_array[i].media_url,open_media_window));
