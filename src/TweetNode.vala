...
class TweetNode:Grid{
  ...
  private ProfileImageDrawingBox profile_image_d_box;
  ...
  public TweetNode(ParsedJsonObj parsed_json_obj,OAuthProxy api_proxy,Config config,SignalPipe signal_pipe){
    ...
    profile_image_d_box=new ProfileImageDrawingBox(parsed_json_obj_.screen_name,parsed_json_obj_.profile_image_url,parsed_json_obj_.account_is_protected,config_,signal_pipe_);
    name_d_box=new NameDrawingBox(parsed_json_obj_.screen_name,parsed_json_obj_.name,config_,signal_pipe_);
    ...
    this.attach(profile_image_d_box,0,0,1,2);
